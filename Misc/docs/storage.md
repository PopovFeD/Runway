...
`storage-and-logs-decision.md` — выполнены все 4 шага).
...
Вкладка «Логи» в GUI читает `events` по галочкам-фильтрам (Info/Warning/Error * «только текущая сессия») по кнопке «Обновить»; экспорт в CSV использует ровно
ту же выборку — «что видишь, то и экспортируешь» (файл в `exports/` рядом с
приложением). Живой нефильтрованный поток — на Дашборде.
...
* фильтруемое чтение событий (`ReadEvents(levels, sessionId)`; null-набор — все уровни, пустой — ничего);
* чтение телеметрии `ReadTelemetry(sessionId)` и счётчик `CountTelemetry()` — для вкладки «Экспорт» и тестов.
...
* **Export** (`Core/Export`) — потребитель `ReadTelemetry`/`CountTelemetry`:
  CSV и минимальный XLSX без сторонних пакетов (см. viewmodels.md, вкладка «Экспорт»).
